...
class Inventory:
    ...
    def add_item(self, item):
        ...
        if len(self.items) >= self.capacity:
            results.append({
                'item_added': None,
                'message': Message('You cannot carry any more, your inventory is full', colors.get('green'))
                })
        else:
            results.append({
                'item_added': item,
    ...
    def use(self, item_entity, **kwargs):
        ...
        if item_component.use_function is None:
            results.append({'message': Message('The {0} cannot be used'.format(item_entity.name), colors.get('dark'))})
        else:
